 ...
 * 제조중인 목록을 보여주는 서블릿
 */
@WebServlet("/pharmacy/dispense/dispensing.do")
public class Dispensing extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 제조중인 목록 가져오기
        ArrayList<DispenseDTO> dtoList = dispenseDAO.getDispensesingPagingList(map);
        ...
        if (cntDto.getJejoCnt() == null) {
            
        } else {            
            totalPage = (int)Math.ceil((double)Integer.parseInt(cntDto.getJejoCnt()) / pageSize);
        }
        ...
        if(n == 1) {
            ...
            sb.append(String.format(("<a href='/apa/pharmacy/dispense/dispensing.do?page=%d'><<<</a>"), n-1, pageSize));
        }
        ...
        while(!(loop > pageSize || n > totalPage)) {
            ...
            if (n == nowPage) {
                ...
                sb.append(String.format(" <a href='/apa/pharmacy/dispense/dispensing.do?page=%d'>%d</a> ", n, n));
            }
        ...
        if(n > totalPage) {
            ...
            sb.append(String.format(("<a href='/apa/pharmacy/dispense/dispensing.do?page=%d'>>>></a>"), n, pageSize));
        }
        ...
        // JSP 페이지로 포워딩
        RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/pharmacy/dispense/dispensing.jsp");
